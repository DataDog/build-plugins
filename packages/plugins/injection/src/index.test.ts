...
const ENTRY_FILES = ['main.js', 'app1.js', 'app2.js'] as const;
...
type HardExpectation = Assign<
    ...
    {
        logs?: {
            'app1.js': ExpectedValues;
            'app2.js': ExpectedValues;
            'chunk-*.js'?: ExpectedValues;
        };
    }
>;
type BuildState = {
    ...
    fullContent?: string;
    // Content of entry files only (for chunk testing)
    entryContent?: string;
    // Content of non-entry chunk files (for chunk testing)
    chunkContent?: string;
    chunkNumber?: number;
    // Separate logs based on executed file.
    logs?: Partial<Record<File, string>>;
};
type File = (typeof ENTRY_FILES)[number];
enum ContentType {
    CODE = 'code',
    LOCAL = 'local file',
...
describe('Injection Plugin', () => {
    ...
    describe('Builds', () => {
        ...
        // Injections + expectations for injectIntoAllChunks (hard build only).
        // This is generated by extending the existing combinator loop below, so it stays consistent.
        const allChunksInjections: ToInjectItem[] = [];
        const hardWithAllChunksInjections: HardExpectation[] = [];
        ...
        for (const type of Object.values(ContentType)) {
            ...
            for (const position of Object.values(Position)) {
                ...
                // Fill in the expectations + injections for all-chunks injection (hard build only).
                // MIDDLE position doesn't support injectIntoAllChunks.
                if (position !== Position.MIDDLE) {
                    hardWithAllChunksInjections.push({
                        name: `[${position}] ${type} all-chunks injection in hard build`,
                        // numbers of expected logs depends on the chunks number the bundler generates
                        // webpack tryes to minimize while rollup does not that why the expected logs range is wider
                        logs: {
                            'app1.js': [injectionLog, [2, 5]],
                            'app2.js': [injectionLog, [1, 5]],
                        },
                        // Higher count because it appears in entries AND chunks.
                        // NOTE: chunk count varies per bundler (runtime/vendor chunks, etc.).
                        content: [injectionContent, [1, 5]],
                    });

                    allChunksInjections.push({
                        type: injectType,
        ...
        const tests: {
            ...
            {
                name: 'Hard build with injections in all chunks',
                entry: hardProjectEntries,
                positions: [Position.BEFORE, Position.AFTER],
                // 3 content types × 2 positions (BEFORE/AFTER only) = 6
                injections: [allChunksInjections, 6],
                expectations: hardWithAllChunksInjections,
                allChunks: true,
            },
        ];
        ...
        const prepareTestRun = async (test: (typeof tests)[number]) => {
            ...
            for (const bundler of BUNDLERS) {
                ...
                // Identify entry files vs chunk files using the entry keys (e.g. ['app1', 'app2']).
                const isEntryFile = (filepath: string): filepath is File => {
                    const basename = path.basename(filepath) as File;
                    return ENTRY_FILES.includes(basename);
                };
                const entryFiles = builtFiles.filter(isEntryFile);
                const chunkFiles = builtFiles.filter((f) => !isEntryFile(f));

                // Run the files through node to confirm they don't crash and assert their logs.
                proms.push(
                    ...entryFiles.map(async (file) => {
                        const basename = path.basename(file) as File;
                        const result = await execute('node', [path.resolve(outdir, basename)]);
                        buildState.logs = buildState.logs || {};
                        buildState.logs[basename] = result.stdout;
                    }),
                );

                // Store separated content for chunk assertions.
                buildState.fullContent = builtFiles
                    .map((file) => readFileSync(file, 'utf8'))
                    .join('\n');
                buildState.entryContent = entryFiles
                    .map((file) => readFileSync(file, 'utf8'))
                    .join('\n');
                buildState.chunkContent = chunkFiles
                    .map((file) => readFileSync(file, 'utf8'))
                    .join('\n');
                buildState.chunkNumber = chunkFiles.length;
            }
        ...
        describe.each(tests)('$name', ({ name: testName, injections, expectations, allChunks }) => {
            ...
            describe.each(BUNDLERS)('$name | $version', ({ name }) => {
                test('Should have a buildState.', () => {
                    ...
                    expect(buildState.fullContent).toEqual(expect.any(String));
                });
                ...
                describe.each(expectations)(
                    ...
                    ({
                        ...
                        if (allChunks) {
                            test('Should appear in the entry files and chunks.', () => {
                                const buildState = states[testName].builds[name];
                                const expectation =
                                ...
                                expect(buildState?.entryContent).toRepeatStringTimes(
                                    expectation,
                                    2,
                                );
                                expect(buildState?.chunkContent).toRepeatStringTimes(
                                    expectation,
                                    buildState?.chunkNumber ?? 0,
                                );
                            });
                        } else {
                            test('Should have the expected content in the bundles.', () => {
                                const buildState = states[testName].builds[name];
                                const content = buildState?.fullContent;
                                const expectation =
                                    expectedContent instanceof RegExp
                                        ? expectedContent
                                        : new RegExp(escapeStringForRegExp(expectedContent));
                                expect(content).toBeDefined();
                                expect(content).toRepeatStringTimes(
                                    expectation,
                                    contentOccurencies,
                                );
                            });
