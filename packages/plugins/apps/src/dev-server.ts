 ...
 */
async function buildStdinContents(filePath: string, functionName: string): Promise<string> {
    const lines = [`export * from ${JSON.stringify(filePath)};`];

    // export * does not include the default export, so re-export it as a named export.
    // Read the file to detect if it uses a default export.
    const fileContents = await readFile(filePath, 'utf-8');
    if (/export\s+default\b/.test(fileContents)) {
        lines.push(`export { default as ${functionName} } from ${JSON.stringify(filePath)};`);
    }
...
async function bundleBackendFunction(
    ...
    await esbuild.build({
        stdin: {
            contents: await buildStdinContents(filePath, functionName),
