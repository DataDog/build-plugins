...
const resolveAuthMethod = (value: string | undefined): AuthMethod | undefined => {
    ...
    throw new Error(`apps.authOverride.method must be one of: ${AUTH_METHODS.join(', ')}`);
};
...
export const validateOptions = (options: Options): AppsOptionsWithDefaults => {
    ...
    const method =
        resolveAuthMethod(getDDEnvValue('APPS_AUTH_METHOD')) ??
        resolveAuthMethod(resolvedOptions.authOverride?.method) ??
        'apiKey';
