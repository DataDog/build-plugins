...
describe('Apps Plugin', () => {
    ...
    test('Should write a Vite package without making an upload request', async ({ bundler }) => {
        ...
        const descriptorPath = path.join(fixtureDestination, 'dist', 'datadog-apps-build.json');
        expect(existsSync(archivePath)).toBe(true);
        expect(existsSync(descriptorPath)).toBe(true);

        // Validate the build descriptor handed to apps-cli.
        const descriptor = JSON.parse(await fs.readFile(descriptorPath, 'utf8'));
        expect(descriptor).toEqual({
            schemaVersion: 1,
            bundle: 'datadog-apps-assets.zip',
            identifier: APP_IDENTIFIER,
            name: APP_NAME,
        });
        ...
        expect(files).not.toEqual(
            expect.arrayContaining([
                'frontend/datadog-apps-assets.zip',
                'frontend/datadog-apps-build.json',
            ]),
        );
    });
});
