...
describe('Injection Plugin', () => {
    ...
    const tests: {
    ...
    }[] = [
        {
            ...
            overrides: getNodeSafeBuildOverrides(),
            positions: [],
            injections: [[], 0],
        ...
        },
        {
            ...
            overrides: getNodeSafeBuildOverrides(getComplexBuildOverrides()),
            positions: [],
            injections: [[], 0],
    ...
    describe.each(tests)('$name', ({ overrides, positions, injections, expectations }) => {
        ...
        beforeAll(async () => {
            ...
            // Prepare mock files and mock routes.
            ...
            for (const bundler of BUNDLERS) {
                ...
                if (!buildState || !outdir) {
                    throw new Error(`Broken buildState in beforeAll for ${bundler.name}.`);
                }
