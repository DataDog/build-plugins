...
import type {
    AuthorizationServer as OAuthAuthorizationServer,
    Client as OAuthClient,
    TokenEndpointResponse as OAuthTokenEndpointResponse,
} from 'oauth4webapi';
...
const loadOauth = memoizeAsync(() => import(OAUTH_PACKAGE_NAME) as Promise<OAuthModule>);

// `@napi-rs/keyring` has a CommonJS entry. Load it lazily so API-key uploads
// do not initialize the native credential binding.
const loadKeyring = memoizeAsync(() => import(KEYRING_PACKAGE_NAME) as Promise<KeyringModule>);
...
export const getOAuthConfig = (site: string): OAuthConfig => {
